...
**Answer**: `fread` and `read_csv` are similar, and the slowest is `read.csv` . `read_csv` and `fread` use very similar amount of memory, while `read.csv` uses more memory than `read_csv` and `fread`.
...
**Answer**: The run time is faster, and the memory usage is about 78MB.
...
```{bash}
#| eval: false
zcat < ~/Documents/203B/mimic/hosp/labevents.csv.gz | awk -F, '($4 == 50912 || $4 == 50971 || $4 == 50983 || $4 == 50902 || $4 == 50882 || $4 == 51221 || $4 == 51301 || $4 == 50931) {print $1","$4","$5","$8}' | gzip > labevents_filtered.csv.gz
```
