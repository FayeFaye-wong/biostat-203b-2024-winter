...
ggplot() +
  geom_segment(data = patient_transfers, aes(x = intime, xend = outtime, 
                                             ...
                                             linewidth= ifelse(
                                               transfer_id==icu_ccu_rows$transfer_id, 
                                               2, 1.5),)) +
...
plot <- ggplot(data = admissions_tble) +
  geom_histogram(mapping = aes(x = los),bins = 20) +
...
plot <- ggplot(data = patients_tble) +
  geom_histogram(mapping = aes(x = anchor_age), bins = 20) +
...
summary_by_race_df$race <- rownames(summary_by_race_df)
summary_by_race_df
```
...
summary_by_insurance_df$insurance <- rownames(summary_by_insurance_df)
...
summary_by_marital_status_df$marital_status <- rownames(summary_by_marital_status_df)
...
summary_by_gender_df$gender <- rownames(summary_by_gender_df)
...
```{r}
ggplot(mimic_icu_cohort, aes(x = `White Blood Cells`, y = los)) +
  geom_hex() +
...
```{r}
ggplot(mimic_icu_cohort, aes(x = `Non Invasive Blood Pressure systolic`, y = los)) +
  geom_hex() +
...
```{r}
ggplot(mimic_icu_cohort, aes(x = `Non Invasive Blood Pressure diastolic`, y = los)) +
  geom_hex() +
...
```{r}
ggplot(mimic_icu_cohort, aes(x = `Respiratory Rate`, y = los)) +
  geom_hex() +
...
summary_by_first_careunit_df$first_careunit <- rownames(summary_by_first_careunit_df)
