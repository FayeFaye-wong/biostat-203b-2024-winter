...
```
The number of patients listed in the `patients.csv.gz` file is 299,712. The number of patients listed in the `patients.csv.gz` file does not match the number of unique patients in the `admissions.csv.gz` file.
```{bash}
...
The running time of `zcat < ~/mimic/labevents.csv.gz | wc -l` is about 0m19s The running time of `wc -l labevents.csv` is about 0m8s. The compressed gz file size is 1.94G. The uncompressed file size is 13.73G. The trade off between storage and speed for big data files is that the compressed gz file size is smaller than the uncompressed file size, but the running time of `zcat < ~/mimic/labevents.csv.gz | wc -l` is longer than the running time of `wc -l labevents.csv`.
...
<p align="center">
  <img src="/Users/huangfeifei/Documents/203B/project/203b-hw/hw1/section415.png" height="450">
</p>